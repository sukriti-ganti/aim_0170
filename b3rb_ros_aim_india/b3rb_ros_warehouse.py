...
def camera_image_callback(self, message):
    if not self.armed:
        return  # Skip QR decoding until robot is armed
    ...
    if decoded_text:
        ...
        self.logger.info(f"QR code decoded: {self.qr_code_str}")
        ...
        shelf_msg.object_name = self.shelf_objects_curr.object_name
        shelf_msg.object_count = self.shelf_objects_curr.object_count
        self.publisher_shelf_data.publish(shelf_msg)

        self.logger.info(f"Published shelf data with QR: {self.qr_code_str}")
